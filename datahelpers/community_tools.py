...
def cut_edges(edges, nnodes=False, nedges=False, verbose=False):
    ...
    g_edges = list(
        map(lambda x: (prot_to_int[x[0]], prot_to_int[x[1]]), edges2[[up, dn]].values)
    )
...
def produce_cluster_df(
    edges_df, cutoff_frac=0.1, unique_edges=True, cut_nodes=False, verbose=False
):
    ...
    dff = edges_df.merge(
        domain_coding_df[["id", "domain_id"]], how="inner", left_on=up, right_on="id"
    )
    del dff["id"]
    dff2 = dff.merge(
        domain_coding_df[["id", "domain_id"]],
        how="inner",
        left_on=dn,
        right_on="id",
        suffixes=("_up", "_dn"),
...
def split_communities_cluster(
    edges, weights=None, cutoff_frac=0.1, directed=True, verbose=False
):
    """
    ...
    if verbose:
        ...
        print("number of connected components {0}".format(len(cc)))
        print(
            "number of disconnected components with > {0} "
        ...
        print(
            "sum of small disconnected components: {0}, "
            "frac of total number of nodes {1:.3f}".format(
                sum_small_disconnected, float(sum_small_disconnected) / len(g.vs)
            )
        )
    ...
    for c in cc_to_communize:
        ...
        if weights:
            ...
            w_g0 = [
                w_dict[(g0_node_g_node_conv[a], g0_node_g_node_conv[b])]
                for a, b in ee_g0
            ]
        ...
        mbsp_agg.extend(
            [
                (g0_node_g_node_conv[k], mbsp[k] + community_count)
...
def prepare_graph_from_df(
    df, file_format="matrix", directed=False, percentile_value=None, verbose=False
):
    ...
    if file_format == "matrix":
        ...
        if verbose:
            print(
                "max of ups: {0}; max of dns: {1}".format(
                    max(list(ups)), max(list(dns))
                )
            )
        uni_nodes = list(set(ups) | set(dns))
        n_uniques = len(uni_nodes)
        ...
        if verbose:
            print(
                "max of renamed columns: {0}; max of renamed index: {1}".format(
                    max(df2.columns), max(df2.index)
                )
            )
        df2 = df2.stack()
        df2 = df2.abs()
...
def prepare_graphdf(
    full_fname, file_format="matrix", key_hdf_store=False, verbose=False
):
    if file_format == "matrix":
        ...
        if isinstance(full_fname, str) and not isinstance(full_fname, list):
            ...
            for f in full_fname:
                if key_hdf_store:
                    ...
                    if verbose:
                        print(
                            "opened {0} by key {1}. Shape is {2}".format(
                                f, key_hdf_store, df_.shape
                            )
                        )
                else:
...
def graph_to_comms(
    ...
    if verbose:
        print(
            "compute took {0} sec; number of communities: {1}".format(
                total_seconds, len(communities)
            )
        )
        lens = sorted([len(c) for c in communities])
        print("Largest 5 are {0}".format(lens[-5:]))
        print(
            "3 top elements from 3 comms are {0}".format(
                [x for x in communities.membership[:3]]
            )
        )
    ...
    if key_hdf_store:
        h5_fname = "{0}_comm_{1}_{2}_{3}_p{4}.h5".format(
            origin, method, directedness, weighted, percentile_value
        )
        if verbose:
        ...
        fout_name = "{0}_comm_{1}_{2}_{3}_p{4}.csv.gz".format(
            origin, method, directedness, weighted, percentile_value
        )
        if verbose:
...
def calculate_comms(
    ...
    g, ws, inv_conversion_map = prepare_graph_from_df(
        df, file_format, directed, percentile_value, verbose
    )
    dt = graph_to_comms(
        g,
        ws,
...
def get_community_fnames_cnames(mode="lincs", storage_type="csv.gz"):
    ...
    zargs = [
        {k: v for k, v in zip(keys2, p)} for p in zip(*(weighteds, percentile_values))
    ]
    origins = [{"origin": "{0}".format(t)} for t in types]

    inv_args = {"fpath_out": "~/data/kl/comms/", "file_format": "matrix"}
    ...
    if mode == "lincs":
        ...
        targs2 = list(
            filter(
                lambda x: not (
                    x["method"] == "infomap"
                    and x["directed"] is False
                    and not x["percentile_value"]
                ),
                targs2,
            )
        )
    ...
    for aa in targs2:
        ...
        fout_name = "{0}_comm_{1}_{2}_{3}_p{4}.{5}".format(
            aa["origin"],
            aa["method"],
            directedness,
            weighted,
        ...
        cname_full = "{0}_comm_{1}_{2}_{3}_p{4}{5}".format(
            aa["origin"], mname, directedness, weighted, percentile_value, suffix
        )
