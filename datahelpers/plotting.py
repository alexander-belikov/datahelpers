...
def plot_hist_true_false(
    dft,
    N=5,
    fname=None,
    x_column_name=ye,
    y_column_name="negative",
    normed_flag=False,
    title="",
    alpha=0.7,
    sns_style="darkgrid",
...
def plot_hist_float_x(
    data, xranges, yranges=None, N=10, opacity=0.8, ylog_axis=False, fname=None
):
    """
...
def plot_hist(
    ...
    for arr in arr_list:
        tmp_arr = min(arr) + (1.0 - 1e-8) * (arr - min(arr))
...
def plot_beta_steps(
    ...
    for j in range(n_features_ext):
        ...
        pps_der = {
            dict_rename[k]: raw_best_dict_plot_beta[k] for k in dict_rename.keys()
        }
        # pprint(pps_der)
        ...
        if fname_prefix:
            ...
            plt.savefig(
                "%s%s_%s%s.%s"
                % (path, fname_prefix, "bestfit_feature_", str(j), format)
            )
    ...
    for j in range(n_features_ext):
        ...
        if fname_prefix:
            ...
            plt.savefig(
                "%s%s_%s%s.%s"
                % (path, fname_prefix, "fits_stats_feature_", str(j), format)
            )
...
def matrix_hist(df, ccs, nrows=2, title=None, xmax=None):
    ...
    ccs_matrix = [
        ccs[nrows * k : nrows * (k + 1)] for k in range(int(np.ceil(len(ccs) / nrows)))
    ]
    n_columns = len(ccs_matrix)

    fig, axx = plt.subplots(
        n_columns, nrows, sharey=False, figsize=(3 * nrows, 3 * n_columns)
    )
