...
def xor(df, ccs, c):
    ...
    print(
        "number of negative claims on reduced statements {0}".format(
            df2.shape[0] - sum(df2[c])
        )
    )
    return df2
...
# drop unneccasary merges
version = 14


dfi = pd.read_pickle("~/data/kl/raw/val_geneways_cs_0.pgz", compression="gzip")

df_ha = pd.read_csv(
    "~/data/kl/raw/human_action.txt.gz", sep="\t", index_col=None, compression="gzip"
)

df_pmid = pd.read_pickle("~/data/kl/raw/medline_doc_cs_2.pgz", compression="gzip")

df_affs = pd.read_csv(
    "~/data/kotta/affiliations/aff_rating.csv.gz", compression="gzip"
...
version2actions = {8: "", 9: "_v2", 10: "", 11: "", 12: "", 13: "", 14: ""}

with open(
    expanduser("~/data/kl/claims/actions{0}.json".format(version2actions[version])), "r"
) as fp:
...
dfr = pd.merge(dfi[dfi_important_columns], df_ha[ha_important_columns], on=hi)
...
with gzip.open(
    expanduser("~/data/kl/claims/df_{0}_{1}_full.pgz".format(origin, version)), "wb"
) as fp:
    pickle.dump(dfr, fp)
...
df_bint_est = dfc.groupby([up, dn, pm], group_keys=False).apply(
    lambda g: np.sum(g[sn_bint] * g[sc + "_shifted"]) / g[sc + "_shifted"].sum()
)
df_bint_est = df_bint_est.reset_index().rename(columns={0: "bint_est"})

df_score_est = dfc.groupby([up, dn, pm], group_keys=False).apply(lambda g: g[sc].sum())
df_score_est = df_score_est.reset_index().rename(columns={0: "score_est"})

dfw = df_bint_est.merge(df_score_est, on=[up, dn, pm])

dfw = dfw.merge(
    dfc[[up, dn, pm, at_orig, from_abstract, "plo"]], on=[up, dn, pm], how="left"
)

# claims pos/neg, from abstracts, with known precision / without features glued
with gzip.open(
    expanduser("~/data/kl/claims/df_{0}_{1}_cut_raw.pgz".format(origin, version)), "wb"
) as fp:
    pickle.dump(dfw, fp)
if version < 14:
    ...
    print(
        "{0} issns in pmids-issn table that are ai table".format(
            len(set_pmids_issns & set_ai_issns)
        )
    )
    ...
    print(
        "{0} of pmids from geneways that are not in pmid-issn table".format(
    ...
    # drop pmids without years
    df_pmid = df_pmid.loc[~df_pmid["year"].isnull()]
    dfw3 = pd.merge(dfw, df_pmid, on=pm)

# claims pos/neg, from abstracts, with known precision / with features glued
with gzip.open(expanduser(f"~/data/kl/claims/df_{origin}_{version}.pgz"), "wb") as fp:
    pickle.dump(dfw3, fp)
