import argparse
import pandas as pd
from datahelpers.dftools import (
    extract_idc_within_frequency_interval,
    count_elements_smaller_than_self_wdensity,
)
...
def main(
    ...
    with gzip.open(
        expanduser("~/data/kl/claims/df_{0}_{1}.pgz".format(df_type, version)), "rb"
    ) as fp:
        df = pickle.load(fp)
    ...
    ids = extract_idc_within_frequency_interval(
        df, ni, ps, (low_freq, hi_freq), low_bound_history_length
    )
    if test_head > 0:
        ids = ids[:test_head]
    ...
    if "density" in feature_groups:
        for w in windows:
            for rc_flag in right_conts:
                df_ = df.groupby(ni).progress_apply(
                    lambda x: count_elements_smaller_than_self_wdensity(
                        x[ye], w, rc_flag
                    )
                )
    ...
    if "ksst" in feature_groups:
        for w in windows:
            for rc_flag in right_conts:
                df_ = df.groupby(ni).progress_apply(
                    lambda x: calculate_uniformity_ks(x[ye], w, rc_flag)
                )
                df = pd.merge(df, df_, how="left", left_index=True, right_index=True)
    ...
    if "density" in feature_groups and "normeddensity" in feature_groups:
        ...
        df_cc, rep = produce_cluster_df(
            df_edges, cutoff_frac=0.0, unique_edges=False, cut_nodes=False, verbose=True
        )
        ...
        df_dd_cnt = df_cc.groupby([dup, ddn], group_keys=False).apply(
            lambda x: x["weight"].sum()
        )
    ...
    max_ye = (
        df.loc[mask_ids].groupby(ni).apply(lambda x: x[ye].max()).rename("max_year")
    )
    min_ye = (
        df.loc[mask_ids].groupby(ni).apply(lambda x: x[ye].min()).rename("min_year")
    )
    diff_ye = (
        df.loc[mask_ids]
        .groupby(ni)
        .apply(lambda x: x[ye].max() - x[ye].min())
        .rename("delta_year")
    )

    df_stats = pd.concat(
        [up_dn, means, sizes, max_ye, min_ye, diff_ye], axis=1
    ).reset_index()
    ...
    if test_head < 0:
        df_stats.to_csv(
            expanduser(
                "~/data/kl/claims/pairs_freq_{0}_v_{1}_n_{2}"
                "_a_{3}_b_{4}.csv.gz".format(
                    df_type, version, low_bound_history_length, low_freq, hi_freq
                )
            ),
            compression="gzip",
            index=True,
    ...
    for idc in ids:
        ...
        if len(data_dict) % n_notify == 0:
            print(
                "{0:.3f}% fraction processed. {1}".format(
                    100 * len(data_dict) / len(ids), len(data_dict)
                )
            )
    ...
    print(
        "number of batches in partition {0}; number of subsamples {1}".format(
            idc_partition, n_samples
        )
    )
    ...
    lens_ = [
        [sub_dict[k].shape[1] for k in sub_dict.keys()] for sub_dict in data_batches
    ]
    ...
    print(
        "products(+1) {0}".format(
            sorted(list(map(lambda x: sum(x) * (len(x) + 1), lens_)))
        )
    )

    datatype = "_".join(columns)

    str_to_hash = "{0}_{1}_{2}_{3}_{4}_{5}_{6}".format(
        df_type,
        version,
        datatype,
        n_samples,
    ...
    if test_head < 0:
        with gzip.open(
            expanduser(
                "~/data/kl/batches/data_batches_{0}_v_{1}"
                "_hash_{2}.pgz".format(df_type, version, datatype_hash_trunc)
            ),
            "wb",
        ) as fp:
    ...
    if test_head < 0:
        fname = expanduser(
            "~/data/kl/batches/df_{0}_v_{1}_hash_{2}.h5".format(
                df_type, version, datatype_hash_trunc
            )
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-d", "--datasource", default="gw", help="type of data to work with [gw, lit]"
    )
    ...
    parser.add_argument(
        "-n", "--minsize-sequence", default=20, type=int, help="version of data source"
    )
    ...
    parser.add_argument(
        "--test", type=int, default=-1, help="test on the head of the dataset"
    )
