...
df = pd.read_csv(
    expanduser("~/data/literome/pathway-extraction.txt.gz"),
    sep="\t",
    compression="gzip",
)
...
df[dn] = df[dn].apply(lambda x: x.lower())
...
print(dfs.shape)
dfi2 = pd.merge(
    dfs, df[[pm, rtype, sentence, upstr, dnstr]], left_on="index", right_index=True
)
print("after pmid remerge:", dfi2.shape)

# convert symbols to entrez id
gc = bgc.GeneIdConverter(
    expanduser("~/data/chebi/hgnc_complete_set.json.gz"), bgc.types, bgc.enforce_ints
)
...
dfi2__ = pd.merge(
    dfi2_,
    dfi2[[pm, up, dn, ps, rtype, sentence, upstr, dnstr]],
    on=[pm, up, dn, ps],
    how="left",
)
...
print(
    "{0} of pmids from literome are not in pmid-issn table".format(
...
print(
    "{0} ai value imputed. frac {0:.3f}".format(
...
# affiliations
df_affs = pd.read_csv(
    expanduser("~/data/kotta/affiliations/aff_rating.csv.gz"), compression="gzip"
...
with gzip.open(
    expanduser("~/data/kl/claims/df_{0}_{1}.pgz".format(origin, version)), "wb"
) as fp:
    pickle.dump(dfi6[[ni, pm, up, dn, ps, ye, ai, ar]], fp)

if full_statement_flag:
    with gzip.open(
        expanduser("~/data/kl/claims/df_{0}_{1}_fs.pgz".format(origin, version)), "wb"
    ) as fp:
        pickle.dump(
            dfi6[[ni, pm, up, dn, ps, ye, ai, ar, upstr, dnstr, rtype, sentence]], fp
        )
