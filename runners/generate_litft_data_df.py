...
pd.DataFrame(decidables[pm].unique(), columns=[pm]).to_csv(
    expanduser("~/data/literome/literome_fulltext_pmids.csv.gz"), compression="gzip"
)
...
print(
    "{0} ai value imputed, out of {1}. It is {2:.3f} fraction".format(
...
with gzip.open(
    expanduser("~/data/kl/claims/df_litft_{0}.pgz".format(version)), "wb"
) as fp:
    pickle.dump(dfi6, fp)
